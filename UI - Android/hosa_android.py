...
class HosaUI(FloatLayout):
    def __init__(self, **kwargs):
        ...
        Clock.schedule_once(lambda dt: self.request_android_notification_permission(), 1)
    ...
    def request_android_notification_permission(self):
    ...
    def show_android_notification(self):
        ...
        self.request_android_notification_permission()
        ...
        # Get current activity and notification service
        activity = PythonActivity.mActivity
        notification_service = cast(NotificationManager, activity.getSystemService(Context.NOTIFICATION_SERVICE))

        # Create notification channel for Android 8+
        channel_id = "fatigue_alerts"
        if Build.VERSION.SDK_INT >= 26:
            channel = NotificationChannel(channel_id, "Fatigue Alerts", NotificationManager.IMPORTANCE_DEFAULT)
            notification_service.createNotificationChannel(channel)

        # Create intent to open a webpage when clicked
        intent = Intent()
        intent.setAction(Intent.ACTION_VIEW)
        intent.setData(Uri.parse("https://www.sleepfoundation.org/how-sleep-works/fatigue"))
        pending_intent = PendingIntent.getActivity(activity, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT)

        # Build notification
        builder = NotificationBuilder(activity, channel_id) if Build.VERSION.SDK_INT >= 26 else NotificationBuilder(activity)
        builder.setContentTitle("Fatigue Alert!")
        builder.setContentText("Tap to view tips.")
